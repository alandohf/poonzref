...
*
*
* (C) COPYRIGHT INTERNATIONAL BUSINESS MACHINES CORPORATION 2002, 2004.
*     ALL RIGHTS RESERVED.
*
...
Prerequisites
 ...
 2. If you are going to create a new database or use an existing database,
    ...
    ------------    ---------  --  ------------------------------------------
    APP_CTL_HEAP_SZ     1024   no  shape import or export
    APPLHEAPSZ         10242   no  various (also for enable_db)
    LOCKLIST             100  yes  Windows has a lower default value than UNIX
    LOGPRIMARY            10   no  Windows has a lower default value than UNIX
 ...
 3. This demo requires larger buffer pools and table spaces than provided by
    ...
    If these database objects are not created, the demo will prompt you to
...
become a major component of IBM's DB2 Universal Database, which includes
...
The customer, branch and employee tables contain implicit spatial information
in the address-related columns.  The sample uses geocoding to transform the 
implicit address location to explicit location data stored in a ST_Point 
data type.

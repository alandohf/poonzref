...
CREATE PROCEDURE PO_shred (IN purchaseorder XML) LANGUAGE SQL
  BEGIN
    ...
    INSERT INTO PO VALUES(XMLDOCUMENT(purchaseorder));
...
SELECT POID, XMLSERIALIZE(porder as VARCHAR(1028))  FROM PURCHASEORDER!
SELECT CID, XMLSERIALIZE(info as VARCHAR(1028)) FROM CUSTOMER!

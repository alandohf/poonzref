...
http://code.google.com/p/recursion/
What is recursion?
...
I don't think there's a need to explain recursion in a new way, you can find numerous explanation for that (wikipedia or just google it).
...
They (the functions/methods) call themselves before they would return, before they would actually finish their job (maybe not themselves directly, but a function/method that called them before, and not yet returned).
There has to be some kind of condition (so called termination condition) in the recursive calls in order so they will stop sometime and return eventually.
...
Whenever we identify self similarity, we should not always turn to recursion automatically. Profit becomes noticeably bigger when the small pieces are not independent, when the solution of a small piece depends on or requires the solution(s) of another piece(s) (usually the neighbors').

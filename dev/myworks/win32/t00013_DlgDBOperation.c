...
#define CHECKDBSTMTERROR(hwnd,retcode,hstmt) if(SQL_ERROR==retcode){ShowDBStmtError(hwnd,hstmt);return 0;}
	...
	case WM_COMMAND://
		{
			switch(wParam)
			{
				case IDC_BUTTON1:
				{
					...
					//设置连接属性
					retcode = SQLSetConnectAttr(hdbc, SQL_LOGIN_TIMEOUT, (void*)LOGIN_TIMEOUT, 0);
					//连接数据库
					...
					retcode = SQLPrepare(hstmt,(SQLCHAR*)"SELECT id,name from test",SQL_NTS);
					...
					while ( SQLFetch(hstmt) != SQL_NO_DATA_FOUND ){
						...
						SQLGetData(hstmt,1,SQL_C_TINYINT,&id,sizeof(SQLINTEGER),(SQLINTEGER*)NULL);
						SQLGetData(hstmt,2,SQL_C_CHAR,name,sizeof(name)/sizeof(SQLCHAR),(SQLINTEGER*)NULL);
						MessageBoxPrintf("Result","%d:%s\n",id,name);
					}

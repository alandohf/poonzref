...
class Flwor
{
  ...
  static void orderCustDetails(Connection con)
  {
    try
    {
      ...
      String query="XQUERY for $custinfo in db2-fn:xmlcolumn('CUSTOMER.INFO')"+
  ...
  static void conditionalCustDetails1(Connection con, int cid)
  {
    ...
    {
      Statement stmt = con.createStatement();
      String query="select xmlquery('for $customer in $cust/customerinfo"+
  ...
  static void conditionalCustDetails2(Connection con, int cid, String country)
  {
    ...
    {
      Statement stmt = con.createStatement();
      String query="select xmlquery('for $customer in db2-fn:xmlcolumn(\"CUSTOMER.INFO\")/customerinfo"+
                               " where (xs:integer($customer/@Cid) gt $id) and ($customer/addr/@country !=$c)"+
  ...
  static void maxpriceproduct(Connection con)
  {
   try
    {
      Statement stmt = con.createStatement();
      String query="XQUERY let $prod := for $product in db2-fn:xmlcolumn('PRODUCT.DESCRIPTION')"+
                   "/product/description"+
  ...
  static void basicproduct(Connection con, double price)
  {
    ...
    {
      Statement stmt = con.createStatement();
      String query="select xmlquery('for $prod in db2-fn:xmlcolumn(\"PRODUCT.DESCRIPTION\")"+
                   "/product/description return ( if ($prod/price < $price)"+

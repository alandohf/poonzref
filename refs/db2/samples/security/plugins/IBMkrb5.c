...
#define _GSSAPI_H_  /* Make sure gssapi.h is not included on Linux NAS 1.4 */
#define GSS_DLLIMP
#ifdef DB2SUN
#include <IBMNASTK/gssapi/gssapi_krb5.h>
#include <IBMNASTK/ibm_svc/krb5_svc.h>
#else
...
/* Prototypes of GSS-API functions in the gssapi_krb5 library and used by the
 * plugin but not in gssapiDB2.h
 */

OM_uint32 SQL_API_FN gss_import_name
...
OM_uint32 SQL_API_FN gss_acquire_cred
                 (OM_uint32 *,                /* minor_status */
...
OM_uint32 SQL_API_FN gss_inquire_cred
                 (OM_uint32 *,                /* minor_status */
...
OM_uint32 SQL_API_FN gss_inquire_context
                 (OM_uint32 *,                /* minor_status */
                  gss_ctx_id_t,               /* context_handle */
                  gss_name_t *,               /* src_name */
...
/*
 * Functions referenced directly from withing krb5 gssapi library:
 *   gss_release_name
 *   gss_acquire_cred
 *   gss_inquire_context
 *   gss_release_cred
 *
 * gssapi functions requiring a wrapper:
                     ...
                     const char *funcName )
{
   ...
   */
  length = majorBuff.length + minorBuff.length + strlen(funcName) + 6;
  ...
  *pErrorMsgLen = length;
  ...
  if( majorBuff.length > 0 )
  {
    ...
    {
      snprintf( *ppErrorMsg, length, "%s: %.*s.  %.*s",
                   funcName, (int)majorBuff.length, (char *)majorBuff.value,
                   (int)minorBuff.length, (char *)minorBuff.value );
    }
    else
    {
      snprintf( *ppErrorMsg, length, "%s: %.*s",
                   funcName, (int)majorBuff.length, (char *)majorBuff.value );
    }
  }
  else
  {
    ...
    {
      snprintf( *ppErrorMsg, length, "%s: %.*s",
                   funcName, (int)minorBuff.length, (char *)minorBuff.value );
    }
    else
    {
      snprintf( *ppErrorMsg, length, "%s:", funcName );
    }
  }
                                                 ...
                                                 db2int32 *pErrorMsgLen )
{
  ...
  krb5_int32 seconds;
  krb5_int32 useconds;
  krb5_flags kdcFlags = KDC_OPT_FORWARDABLE;
  int retryGrabTGT = 0;
  int x = 0;
  ...
  */
  ...
  snprintf( nameSeed, sizeof(nameSeed), "%d%d"OSS_PRIUintPtr,
            seconds, useconds, (UintPtr)(char *)pGSSCredHandle  );
 ...
 grabTGT:
  /* Use the password to grab a forwardable TGT and place in the new cred
   * cache */
  kStatus = krb5_get_in_tkt_with_password( kContext,
                                           kdcFlags,
                                           NULL,   /* Addresses */
                                           NULL,   /* Encryption type */
                                           NULL,   /* Preauthentication */
                                           password, 
                                           cCache,
                                           &kCreds,
                                           NULL ); /* KDC reply */

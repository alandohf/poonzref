...
#if (   defined(DB2AIX) || defined(DB2SCO) || defined(DB2SUN) ||        \
        defined(DB2HP) || defined(DB2SNI) || defined(DB2IRIX) ||        \
        defined(DB2LINUX))
#define _NOW32OS
#else
#define _W32OS
...
int createDb(char* dbase)
{
  ...
  sqlecrea(dbase, dbase, "", &db_desc, &dbCountry, '\0', NULL, &sqlca);
...
/*                           BuildGCParString()                         */
/*                                                                      */
/* -Just build the parameters string for the default geocoder UDF       */
/* -parvalues - point to a string where the parameters will be placed   */
/*                                                                      */
/*======================================================================*/
void BuildGCParString(char *bparvalues,
                      char *street,char *city,char *state,char *zip,
                      int srid,int splsens,int minmatch,
                      double sideoffset,char *sideoffunit,int endoffset,
                      char *edgfile,char *locfile)
{
  char homedir[255];

  strcpy(homedir,getenv("DB2PATH"));
  sprintf(bparvalues,"%s,%s,%s,%s,%i,%i,%i,%lf,%s,%i,",
          street,city,state,zip,srid,splsens,
          minmatch,sideoffset,sideoffunit,endoffset);
#ifdef _NOW32OS
  if(edgfile)
  {
    strcat(bparvalues,"'");
    strcat(bparvalues,homedir);
    strcat(bparvalues,"/gse/refdata/");
    strcat(bparvalues,edgfile);
    strcat(bparvalues,"',");
  }
  else
  {
  ...
  if(locfile)
  {
    strcat(bparvalues,"'");
    strcat(bparvalues,homedir);
    strcat(bparvalues,"/samples/extenders/spatial/");
    strcat(bparvalues,locfile);
    strcat(bparvalues,"'");
  }
...
int stEnableDB()
{
  ...
  "CREATE PROCEDURE db2gse.my_enable_db ( "                \
       "IN   tableCreationParameters VARCHAR(32672), "     \
       "OUT  msgCode                 INTEGER, "            \
       "OUT  msgText                 VARCHAR(1024) ) "     \
...
int gseSetupTables()
{
  ...
  struct sqlchar *pColumnString;
  char impStatement[] = "INSERT INTO CUSTOMERS";
  char modStr[] = " chardel?";
  struct sqluimpt_in importInput;
  struct sqluimpt_out importOutput;
  struct sqlchar *pFileTypeMod = (struct sqlchar *)malloc(
    sizeof(struct sqlchar) + strlen(modStr)+1);
  ...
  pColumnString = (struct sqlchar *)malloc(MAX_STMT_LEN
  ...
  importInput.sizeOfStruct = SQLUIMPT_IN_SIZE;
  importOutput.sizeOfStruct = SQLUIMPT_OUT_SIZE;
  importInput.restartcnt = 0;
  importInput.commitcnt = 0;

  /* setup modifier used for file ('?') */
  pFileTypeMod->length = strlen(modStr);
  strcpy(pFileTypeMod->data, modStr);

  /* / do the IMPORT */
  rc = sqluimpr(datafile,        /* name of file to be imported */
                NULL,            /* lob-path (not needed) */
                &dataDescriptor, /* data descriptor (column mapping) */
                pColumnString,   /* "IMPORT INTO" */
                SQL_DEL,         /* delimited ASCII file */
                pFileTypeMod,    /* "CHARDEL?" */
                msgfile,         /* local message file */
                0,               /* caller action */
                &importInput,    /* input info */
                &importOutput,   /* output info */
                NULL,            /* null indicators */
                NULL,            /* reserved */
                &sqlca);
  ...
  if (rc != SQL_ERROR && rc != SQL_INVALID_HANDLE)
  {
    printf("          %d rows read, %d rows committed.\n",
                 importOutput.rowsRead, importOutput.rowsCommitted);
    if(importOutput.rowsCommitted > 0 &&
               importOutput.rowsCommitted == importOutput.rowsRead)
...
/*                                                                      */
/*======================================================================*/
int stImportSDE(char *sourceschema,char *sourcetable,char *sourcecolumn,
                char *path, int commitscope)
{
  SQLRETURN       rc;

  SQLCHAR   callImportSDE[]
    = "call db2gse.gse_import_sde (?,?,?,?,?,?,?)";

  rc = SQLAllocHandle(SQL_HANDLE_STMT,hdbc, &hstmt);
  CHECK_DBC(hdbc, rc);

  rc = SQLPrepare(hstmt, callImportSDE, SQL_NTS);
  CHECK_STMT(hstmt, rc);
  ...
  ind[3] = SQL_NTS;
  rc = SQLBindParameter(hstmt, 4, SQL_PARAM_INPUT, SQL_C_CHAR,
                        SQL_VARCHAR, 128, 0, path, 129, &ind[3]);
  CHECK_STMT(hstmt, rc);

  ind[4] = (commitscope == -1) ? SQL_NULL_DATA : SQL_NTS;;
  rc = SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_LONG,
                        SQL_INTEGER, 0, 0, &commitscope, 4, &ind[4]);
  CHECK_STMT(hstmt, rc);
  ...
  printf("\n==========> Importing the SDE file '%s' ...\n", path);
  fflush(stdout);
  rc = SQLExecute(hstmt);
  CHECK_STMT(hstmt, rc);

  if (retCode != 0)
    printf("<========== Failed to import the SDE file '%s' with return \
code=%d text=%s\n\n", path, retCode, reserved);
  else
    printf("<========== The SDE file '%s' was successfully imported\n\n",
           path);
...
/*                                                                      */
/*======================================================================*/
int stExportSDE(char *sourceschema,char *sourcetable,char *sourcecolumn,
                char *path,char *whereclause)
{
  SQLRETURN       rc;

  SQLCHAR   callExportSDE[]
    = "call db2gse.gse_export_sde (?,?,?,?,?,?,?)";

  rc = SQLAllocHandle(SQL_HANDLE_STMT,hdbc, &hstmt);
  CHECK_DBC(hdbc, rc);

  rc = SQLPrepare(hstmt, callExportSDE, SQL_NTS);
  CHECK_STMT(hstmt, rc);
  ...
  ind[4] = (whereclause == NULL) ? SQL_NULL_DATA : SQL_NTS;
  rc = SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_CHAR,
                        SQL_VARCHAR, 255, 0, whereclause, 256, &ind[4]);
  CHECK_STMT(hstmt, rc);
  ...
  printf("\n==========> Exporting to SDE file '%s' ...\n", path);
  fflush(stdout);
  rc = SQLExecute(hstmt);
  CHECK_STMT(hstmt, rc);

  if (retCode != 0)
    printf("<========== Failed to export the SDE file '%s' with return \
code=%d text=%s\n\n", path, retCode, reserved);
  else
    printf("<========== The SDE file '%s' was successfully exported\n\n",
           path);
...
db2gse.spatial_index(%12.4f,%12.4f,%12.4f)",
          indexname,sourcetable,sourcecolumn,gridsize1,gridsize2,gridsize3);

  printf("%s\n\n",EnableIDXBuffer);

  /* Spatial index support currently exists only in single partition
     databases. We will check the number of instances as listed in
     db2nodes.cfg and block index functions if number of partitions > 1 */
  if (getNumPartitions() > 1) {
    printf("<--------------- Spatial indexes are not supported for multinode \
databases. The create index statement is skipped.\n\n");
    return 0;
  }
...
int gseCLIDropIndex(char *indexname)
{
  ...
  if (getNumPartitions() > 1) {
    printf("<--------------- Spatial indexes are not supported for multinode \
databases. The drop index statement is skipped.\n\n");
    return 0;
  }
...
int gseInsDelUpd()
{
  ...
  SQLCHAR   insertCustomers[]
    = "INSERT INTO CUSTOMERS \
        SELECT ID+100000, 'MICHELLE' || SUBSTR(NAME,5,LENGTH(NAME)-4), \
        SUBSTR(ADDRESS,2,LENGTH(ADDRESS)-1), \
        CITY, STATE, ZIP, INCOME + 1000, PREMIUM + 500, CATEGORY, \
        CAST(NULL AS DB2GSE.ST_POINT) \
        FROM CUSTOMERS \
        WHERE SUBSTR(NAME,1,4) = 'GENE' AND SUBSTR(ADDRESS,1,1) \
        IN ('1','2') AND \
        SUBSTR(ADDRESS,2,1) NOT IN ('0')";

  SQLCHAR   deleteCustomers[]
    = "DELETE FROM CUSTOMERS WHERE ID >=  81000 AND ID <= 140000";

  SQLCHAR   updateCustomers[]
    = "UPDATE CUSTOMERS SET ADDRESS = '1' || ADDRESS WHERE ID >= 140000";
...
/*            xOffset: 562500   xScale : 1000                      */
/*            yOffset: 1708000  yScale : 1000                      */
/*            zOffset: 0        zScale : 1                         */
/*            mOffset: 0        mScale : 1                         */
/*                                                                 */
/*=================================================================*/
int TestSRS(void)
{
  ...
  /* used by spatial extender routines to handle the data in a 32  */
  /* bit integer representation. For the coordinate system we will */
  /* use the KY_STATE_PLANE coordinate system as defined earlier.  */
  /*                                                               */
  /* For the offset values and scale factor two major conditions   */
  /* apply for the SRS we create:                                  */
  /* 1.  minimum x coordinate - xMin > 0                           */
  /*     minium y coordinate - yMin > 0                            */
  /* 3. (maxium x coordinate - xMin) * xOffset < 2^32              */
  /*    (maxium y coordinate - yMin) * yOffset < 2^32              */
  /*                                                               */
  /* The values we picked here are based on                        */
  /* the minium x coordinate possible (file regions.shp) = 562573  */
  /* the minium y coordinate possible (file regions.shp) = 1708027 */
  /* the maxium x coordinate possible (file regions.shp) = 2739247 */
  /* the maxium y coordinate possible (file regions.shp) = 2664796 */
  /*                                                               */
  /* We meet the conditions 1 and 2 with                           */
  /* 1. 562576   -  562500 > 0                                     */
  /*    1708027  - 1708000 > 0                                     */
  /* 2. (2739247 - 562500) * 1E+2 < 2^31                           */
  /*    (2664796 - 170800) * 1E+2 < 2^32                           */
  /*===============================================================*/

  rc = stCreateSRS("KY_PROJ_SRS",100002,562500,100,1708000,100,0,1,0,1,
                   "KY_STATE_PLANE",
...
int TestRegisterGeocoder(void)
{
  ...
  /* that uses the DB2 geocoding function db2gse.ST_EDGE_GC_USA     */
  /* to translate US-address data into lat./long. coordinates.      */
  /* A set of parameters is used to adjust the geocoding function.  */
  /* - column names that match ADDRESS, CITY, STATE and ZIP         */
  /* - the spatial reference system used for the result coordinates */
  /*   is identified by the SRSID 100001                            */
  /* - the spelling sensitivity for the addresses is 80 per cent    */
  /* - the minimal match score of the result coordinates must be 60 */
  /* - the resulting point should be placed with a 0.0 offset from  */
  /*   its actual location                                          */
  /* - the offset value uses FEET as unit of measure                */
  /* - the end offset of 3 will place point of intersections just   */
  /*   slightly beside the actual intersection point                */
  /* - the base map to be used is USA.edg                           */
  /* - additional parameters are specified in file EDGELocator.loc  */
  /*                                                                */
  /* The parameter description is left NULL and the vendor name is  */
  /* DEFAULT. With the description 'Edge Geocoder for USA streets'  */
  /* we have an additional field to distinguish between multiple    */
  /* geocoder.                                                      */
  /*                                                                */
  /*================================================================*/

  rc = stRegisterGc("SAMPLEGC","db2gse","ST_EDGE_GC_USA","ST_USAGEOCODER",
                    "ADDRESS,CITY,STATE,ZIP,100000,80,60,0.0,FEET,3,\
                    'usa.edg','$(DB2PATH)/gse/cfg/geocoder/EDGELocator.loc'",
                    NULL,"DEFAULT","Edge Geocoder for USA streets");
  GseCheck(rc);
  ...
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,80,
                   60,0.0,"'Feet'",3, "ky.edg","EDGESample.loc");

  rc = stRegisterGc("KY_STATE_GC","db2gse","ST_EDGE_GC_USA","ST_USAGEOCODER",
                    parvalues, NULL, "DEFAULT",
                    "Edge Geocoder for Kentucky State Coordinates");
  GseCheck(rc);
  /*=================================================================*/
...
int TestSetupGeocoder(void)
{
  ...
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,90,60,
                   0.0,"'FEET'",3,"ky.edg","EDGESample.loc");
  rc = stSetupGeocoding(NULL, "CUSTOMERS", "LOCATION","KY_STATE_GC",
                        parvalues,NULL,NULL,-1);
  GseCheck(rc);
...
/*========== Testing Geocoding with registered Geocoder ===========*/
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,100,80,
                   0.0,"'FEET'",3,"ky.edg","EDGESample.loc");
...
int TestIndexes(void)
{
  ...
  /* The following data have been generated in a test run on
     ...
     Number of Rows: 343
     Number of non-empty Geometries: 327
  ...
  rc = gseCLICreateIndex("CUSTOMERS","LOCATION","CUSTOMERSLOC",0.459, 0.0,
                         0.0);
  GseCheck(rc);
  ...
  /* The following data have been generated in a test run on
     ...
     Number of Rows: 62
     Number of non-empty Geometries: 62
     Number of empty Geometries: 0
  ...
  rc = gseCLICreateIndex("OFFICES", "LOCATION","OFFICESLOC",1.02, 0.0, 0.0);
  GseCheck(rc);
  ...
  rc = gseCLICreateIndex("FLOODZONES", "LOCATION","BOUNDARYLOC",1.18, 0.0,
                         0.0);
  GseCheck(rc);
  ...
  rc = gseCLICreateIndex("REGIONS", "LOCATION","REGIONLOC",3.19,7.82, 0.0);
  GseCheck(rc);
...
int TestAutoGC(void)
{
  ...
  /*      column                                                        */
  /*  - Here we give another locator file with the edgefile declared    */
  /*  inside, so we don't need to specify it                            */
  /*  - Make sure the EDGESample.loc is in the sqllib/samples/extenders/spatial   */
  /*  and the edge file defined there is ky.edg                         */
  /*====================================================================*/
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,100,60,
                   0.0,"'FEET'",3,NULL,"EDGESample.loc");

  rc = stSetupGeocoding(NULL, "CUSTOMERS", "LOCATION","KY_STATE_GC",
                        parvalues,"ADDRESS,CITY,STATE,ZIP",NULL,-1);
  GseCheck(rc);
  ...
  /*===================================================================*/
  /*  (1) Disable the auto. geocoder for the customers/location column */
  /*  (2) Disable the spatial index for the customers/location column  */
  /*===================================================================*/
  rc = stDisableAutoGc(NULL, "CUSTOMERS", "LOCATION");
  GseCheck(rc);
  ...
  GseCheck(rc);
  /*=================================================================*/
  /*  (1) Geocode the customers/location column again with lower     */
  /*      precision level (90% instead of 100%)                      */
  /*  (2) Re-enable the spatial index for the customers/location     */
  /*      column                                                     */
  /*  (3) Re-enable the automatic geocoder with a lower precision    */
  /*      level (90% instead of 100%)                                */
  /*=================================================================*/
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,90,60,
                   0.0,"'Feet'",3, "ky.edg","EDGESample.loc");
  rc = stRunGC(NULL, "CUSTOMERS", "LOCATION","KY_STATE_GC",parvalues,
               NULL,1);
  GseCheck(rc);

  rc = gseCLICreateIndex("CUSTOMERS", "LOCATION","CUSTOMERSLOC",100.0, 0.0,
                         0.0);
  GseCheck(rc);

  /*=================================================================*/
  BuildGCParString(parvalues,"ADDRESS","CITY","STATE","ZIP",100002,90,60,
                   0.0,"",3,"ky.edg","EDGESample.loc");
  rc = stSetupGeocoding(NULL, "CUSTOMERS", "LOCATION","KY_STATE_GC",
                        parvalues,"ADDRESS,CITY,STATE,ZIP",NULL,-1);
  GseCheck(rc);
...
boundaries:";
  ...
  SQLCHAR query4[] =
    "SELECT FZ.NAME FROM FLOODZONES FZ, REGIONS R1, REGIONS R2 \n \
        WHERE (R1.NAME = 'Kentucky Central' AND \n \
        DB2GSE.ST_OVERLAPS(FZ.LOCATION, R1.LOCATION) = 1) \n \
        AND \n \
        (R2.NAME = 'Kentucky West' AND \n \
...
nearest office:";
  ...
  SQLCHAR query5[] =
    "WITH TEMP(CUSTOMER,ID,OFFICE, DISTANCE) AS \n \
        ...
        DB2GSE.ST_DISTANCE(C.LOCATION, O.LOCATION)\n \
        FROM CUSTOMERS C, OFFICES O \n \
        WHERE C.ID = 80708 ) \n \
        SELECT DISTINCT ID,CUSTOMER, OFFICE, \n \
        DECIMAL(DISTANCE,6,1) AS DISTANCE \n \
        FROM TEMP  \n \
        WHERE DISTANCE = \n \
        (SELECT MIN(DISTANCE) \n \
        FROM TEMP)";
...
/****************************************************************************/
  SQLCHAR query6txt[] = "Customers located within or near flood zone 'Boone':";

  SQLCHAR query6[] =
    "SELECT C.NAME FROM CUSTOMERS C, FLOODZONES FZ \n \
        WHERE FZ.NAME = 'Boone'  AND   \n \
        ((DB2GSE.ST_WITHIN(C.LOCATION, FZ.LOCATION)=1)  OR \n \
        (DB2GSE.ST_INTERSECTS(C.LOCATION, \n \
        DB2GSE.ST_BUFFER(FZ.LOCATION, 0.5, 'STATUTE MILE'))=1) ) ORDER BY C.NAME";
...
/*=================================================================*/
/*         Test Export/Import spatial tables into SDE format       */
/*=================================================================*/
/*                                                                 */
/*  (1) Export the 'Customers' table to sdecustomtest sde file     */
/*  (2) Import the 'sdecustomtest' file to IMPCUSTOMERS            */
/*                                                                 */
/*=================================================================*/
int TestImportExportSDE(void)
{
  char tbuffer[100];
  time_t xtime;

  printf("START: TestImportExportSDE()\n");
  time(&xtime);
  sprintf(tbuffer,"expCustSDE_%i",xtime);
  gseBuildTempFolder(path,tbuffer);

  printf("==========> Export from Customers table to SDE file ...\n");
  fflush(stdout);
  rc = stExportSDE(NULL,"CUSTOMERS","LOCATION",path,NULL);
  GseCheck(rc);

  printf("==========> Import from SDE file to table CustFromSDE ...\n");
  fflush(stdout);
  rc = stImportSDE(NULL,"CUSTFROMSDE","LOCATION",path,-1);
  GseCheck(rc);
...
int menu()
{
  ...
  while(choice!=99)
  {
    ...
    switch(choice)
    {
      ...
      case 14:TestImportExportSDE();                  break;
